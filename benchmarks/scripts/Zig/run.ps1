# run.ps1 — Phase 3 Zig benchmark runner
# Per benchmarks/methodology.md: 3-pass capture (wall-time, stats, RSS).
#
# Usage:
#   pwsh -File benchmarks/scripts/Zig/run.ps1             # full sweep (51 files)
#   pwsh -File benchmarks/scripts/Zig/run.ps1 -SmokeTest  # one cell only (huge_dense × d=8)
#   pwsh -File benchmarks/scripts/Zig/run.ps1 -SkipBuild  # reuse existing zig-out/bin/dijkstra-example.exe
...
$zigExampleDir = Join-Path $repoRoot 'examples\dijkstra\Zig'
$binary        = Join-Path $zigExampleDir 'zig-out\bin\dijkstra-example.exe'
$envFile       = Join-Path $PSScriptRoot '..\env-Zig.json'
$resultsDir    = Join-Path $repoRoot 'benchmarks\results\Zig'

if (-not (Test-Path $envFile)) {
    throw "env-Zig.json not found at $envFile. Run benchmarks/scripts/env.ps1 -Language Zig first."
}
...
# --- Build ---
if (-not $SkipBuild) {
    Write-Host "Building Zig binary (ReleaseFast)..."
    Push-Location $zigExampleDir
    try {
