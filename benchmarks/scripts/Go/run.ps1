# run.ps1 — Phase 3 Go benchmark runner
# Per benchmarks/methodology.md: 3-pass capture (wall-time, stats, RSS).
#
# Usage:
#   pwsh -File benchmarks/scripts/Go/run.ps1             # full sweep (51 files)
#   pwsh -File benchmarks/scripts/Go/run.ps1 -SmokeTest  # one cell only (huge_dense × d=8)
#   pwsh -File benchmarks/scripts/Go/run.ps1 -SkipBuild  # reuse existing dijkstra-example.exe
...
$goExampleDir = Join-Path $repoRoot 'examples\dijkstra\Go'
$binary       = Join-Path $goExampleDir 'dijkstra-example.exe'
$envFile      = Join-Path $PSScriptRoot '..\env-Go.json'
$resultsDir   = Join-Path $repoRoot 'benchmarks\results\Go'

if (-not (Test-Path $envFile)) {
    throw "env-Go.json not found at $envFile. Run benchmarks/scripts/env.ps1 -Language Go first."
}
...
if (-not $SkipBuild) {
    ...
    Push-Location $goExampleDir
    try {
        go build -o dijkstra-example.exe .
