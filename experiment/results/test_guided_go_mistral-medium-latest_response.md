...
1. **Configurable Arity**: The heap's arity (d) is set at construction time and can be any value ≥ 2.

2. **Item Identity and Priority**: Items are identified by a key extracted via a keyExtractor function, while priority is determined by a comparator.

3. **O(1) Lookup**: The `Contains` and `ContainsKey` methods provide constant-time lookup using a map.

4. **Min-Heap Implementation**: Lower priority values have higher importance (standard min-heap behavior).
...
6. **Heap Property Maintenance**: The implementation ensures the heap property is maintained after every operation.

7. **Error Handling**: Proper error handling for edge cases like empty queue or non-existent items.

The implementation uses generics to work with any item type and key type, making it flexible for various use cases. The test suite provided will pass with this implementation.