Looking at the test requirements, I need to implement a d-ary heap priority queue with specific characteristics. Let me analyze the key requirements:

1. Configurable arity (d) at construction
2. Items have separate identity and priority
3. Equality based on identity only
4. O(1) lookup via `contains()`
5. Min-heap (lower priority values = higher importance)
6. Support for priority updates
...
1. **Configurable d-ary heap**: The `d` parameter sets the number of children per node
2. **Separate identity and priority**: Items are compared by identity for equality/hashing, but ordered by priority
3. **O(1) lookup**: The `index_map` HashMap provides constant-time `contains()` checks
4. **Min-heap semantics**: Lower priority values have higher importance
5. **All required operations**: insert, pop, front, peek, increase_priority, decrease_priority, contains, len, is_empty
6. **Proper heap maintenance**: Bubble-up and bubble-down operations maintain the heap invariant
7. **Index tracking**: The HashMap keeps track of item positions for efficient priority updates

The key insight is using a HashMap to map items to their heap indices, enabling O(1) lookups and efficient priority updates. The heap operations properly maintain both the heap structure and the index mapping.