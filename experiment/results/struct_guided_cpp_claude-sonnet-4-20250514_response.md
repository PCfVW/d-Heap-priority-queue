Here's the complete implementation of the d-ary heap priority queue:
...
1. **D-ary heap structure**: Uses the standard formulas for d-ary heaps:
   - Parent of node at index `i`: `(i-1)/d`
   - First child of node at index `i`: `d*i + 1`

2. **O(1) lookup**: The `positions_` hash map maintains the mapping from item ID to its current index in the heap array.

3. **Heap maintenance**: 
   - `heapify_up()`: Used after insertion and priority increases
   - `heapify_down()`: Used after removal and priority decreases
   - Both functions update the positions map when swapping elements

4. **Min-heap property**: Lower priority values (higher importance) bubble up to the root.

5. **Exception safety**: Throws appropriate exceptions for invalid operations as specified.

6. **Template flexibility**: Supports custom hash and equality functors for the key type.

The implementation maintains all heap invariants while providing O(1) lookup through the hash map, making operations like `contains()`, `increase_priority()`, and `decrease_priority()` efficient.