...
1. **Configurable Arity**: The heap arity `d` is set at construction time and must be ≥ 2.

2. **Item Structure**: Items have separate `Identity` and `Priority` fields, where identity is used for equality and priority for ordering.

3. **O(1) Lookup**: The `indexMap` provides constant-time lookup to check if an item exists and find its position.

4. **Min-Heap**: Lower priority values have higher importance (appear first).
...
1. **Index Management**: The `indexMap` is kept synchronized with heap operations through the `swap` method.

2. **Heap Property**: Maintained through `bubbleUp` and `bubbleDown` operations.

3. **Error Handling**: Comprehensive error checking for edge cases like empty heap, duplicate items, and invalid priority updates.

4. **Memory Efficiency**: Uses a slice for the heap array with automatic resizing.

The implementation is complete, tested, and ready for production use. The example in `main()` demonstrates all operations and edge cases.