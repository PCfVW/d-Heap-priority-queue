# D-ary Heap Priority Queue Implementation in Go
...
1. **D-ary Heap Structure**: The heap has `d` children per node, allowing for different time complexity tradeoffs.

2. **O(1) Lookup**: The `positions` map provides constant-time lookup to check if an item exists.

3. **Min-Heap Property**: Lower priority values have higher importance (are at the top of the heap).

4. **Efficient Operations**:
   - Insert: O(log_d n)
   - Pop: O(d log_d n)
   - Increase/Decrease Priority: O(log_d n)
   - Contains: O(1)

5. **Error Handling**: Proper error handling for duplicate inserts and missing items during priority updates.

## Example Usage:

```go
func main() {
	pq := New[string](3) // ternary heap

	// Insert items
	pq.Insert(Item[string]{ID: "task1", Priority: 5})
	pq.Insert(Item[string]{ID: "task2", Priority: 3})
	pq.Insert(Item[string]{ID: "task3", Priority: 7})

	// Check front
	front, _ := pq.Front()
	fmt.Println("Front item:", front) // Should be task2 with priority 3

	// Increase priority (lower value)
	pq.IncreasePriority(Item[string]{ID: "task3", Priority: 1})

	// Check front again
	front, _ = pq.Front()
	fmt.Println("Front item after increase:", front) // Should be task3 with priority 1
...
This implementation provides all the required functionality while maintaining the heap properties and efficient operations.