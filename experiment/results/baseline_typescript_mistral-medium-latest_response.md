...
1. **Configurable Arity**: The heap's arity (d) is set during construction and must be at least 2.

2. **Item Structure**: Items have both an identity (for equality) and a priority (for ordering).

3. **O(1) Lookup**: Uses a Map to track item positions for constant-time contains checks.
...
```typescript
const queue = new DaryHeapPriorityQueue<string>(3); // 3-ary heap

queue.insert({ identity: "task1", priority: 5 });
queue.insert({ identity: "task2", priority: 2 });
queue.insert({ identity: "task3", priority: 8 });

console.log(queue.front()); // { identity: "task2", priority: 2 }

queue.increasePriority({ identity: "task3", priority: 8 }, 1);
console.log(queue.pop()); // { identity: "task3", priority: 1 }

console.log(queue.contains("task1")); // true
console.log(queue.len()); // 2
```

This implementation provides all the required functionality while maintaining the heap properties and ensuring efficient operations.