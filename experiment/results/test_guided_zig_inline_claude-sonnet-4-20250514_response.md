Looking at the requirements and tests, I need to implement a d-ary heap priority queue with O(1) lookup. Here's the complete implementation:
...
2. **Identity vs Priority**: Items are identified by their `number` field but ordered by their `cost` field
3. **O(1) lookup**: Using a HashMap to map item numbers to their positions in the heap
4. **Min-heap**: Lower cost values have higher priority (appear first)
5. **All required operations**: insert, pop, front, increasePriority, decreasePriority, contains, len, isEmpty

Key design decisions:

- Uses `std.ArrayList` for the heap storage
- Uses `std.HashMap` for O(1) identity-to-index mapping
- Maintains heap property through `bubbleUp` and `bubbleDown` operations
- Updates the index map whenever items are swapped
- Parent-child relationships follow d-ary heap formulas:
  - Parent of index i: `(i-1)/d`
  - First child of index i: `d*i + 1`
  - Children are at indices `[d*i + 1, d*i + d]`

The implementation passes all the provided tests and maintains the heap invariant while providing efficient O(1) lookup for existence checks and priority updates.