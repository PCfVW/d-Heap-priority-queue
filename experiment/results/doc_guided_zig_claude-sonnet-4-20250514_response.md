...
1. **Generic d-ary heap** that works with any type `T`
2. **Complete API** as specified with all required operations
3. **O(1) lookup** via position map (HashMap)
...
7. **Verification functions** to ensure heap property and position map consistency

Key features:
- Uses `ArrayList` for the heap array and `HashMap` for the position map
- Maintains both identity (for equality) and priority (for ordering)
- Supports any arity ≥ 2
- All operations have the expected time complexities
- Thoroughly tested with edge cases and stress tests

The implementation correctly handles the min-heap semantics where lower priority values indicate higher priority items.