...
1. **Configurable Arity (d)**: The heap's arity is set during construction and used for all heap operations.

2. **Item Identity and Priority**: Items are compared based on their priority, but identity is used for equality checks.

3. **O(1) Lookup**: The `keyToIndexMap` provides constant-time lookup to check if an item exists.

4. **Min-Heap**: The implementation maintains the min-heap property where the smallest priority value is at the root.
...
5. **All Required Operations**:
   ...
   - `increasePriority(item)`: Updates an item's priority to be higher (lower value)
   - `decreasePriority(item)`: Updates an item's priority to be lower (higher value)
   - `contains(item)`: Checks if an item exists
   - `len()`: Returns the number of items
   - `is_empty()`: Checks if the heap is empty

6. **Heap Maintenance**:
   - `bubbleUp()`: Moves an item up the heap when its priority increases
   - `bubbleDown()`: Moves an item down the heap when its priority decreases
   - `swap()`: Swaps two items and updates their positions in the index map

The implementation handles all edge cases mentioned in the tests, including empty heap operations and proper error throwing when items aren't found.