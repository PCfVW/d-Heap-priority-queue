Here's a complete implementation of the d-ary heap priority queue in Go:
...
1. **D-ary Heap Structure**: The heap uses a slice where for node at index `i`:
   - Parent is at `(i-1)/d`
   - First child is at `d*i + 1`
   - Children are at indices `d*i + 1` through `d*i + d`

2. **O(1) Lookup**: The `positions` map provides constant-time lookup by mapping item IDs to their indices in the container.

3. **Heap Operations**:
   - `bubbleUp`: Moves an item up until heap property is satisfied
   - `bubbleDown`: Moves an item down, comparing with all d children to find the minimum

4. **Priority Updates**: 
   - `IncreasePriority` (lower value) only needs to bubble up
   - `DecreasePriority` (higher value) only needs to bubble down

5. **Position Tracking**: The positions map is kept synchronized with every swap operation.

The implementation maintains the min-heap property where the root always contains the item with the lowest priority value (highest importance), and provides efficient O(log_d n) operations for insertion, deletion, and priority updates.