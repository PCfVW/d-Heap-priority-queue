Here's the complete implementation of the d-ary heap priority queue in Zig:
...
1. **Complete d-ary heap functionality** with configurable arity
2. **O(1) lookup** via the HashMap that maps items to their positions
3. **Identity-based equality** where items are equal if they have the same `number`
4. **Priority-based ordering** where lower `cost` means higher priority
5. **Efficient operations**:
   - Insert: O(log_d n)
   - Pop: O(d log_d n)
   - Priority updates: O(log_d n)
   - Contains: O(1)
6. **Proper error handling** for duplicate inserts and missing items
7. **Memory management** with proper cleanup in `deinit()`

The key features include:
- The `positions` HashMap maintains the mapping from items to their current indices
- `heapifyUp` and `heapifyDown` maintain heap properties after modifications
- The `swap` function updates both the container and the positions map
- Priority updates properly handle both increasing (bubbling up) and decreasing (bubbling down) priority