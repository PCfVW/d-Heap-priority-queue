    ...
    C: PriorityCompare<T>,
{
    ...
    pub fn pop(&mut self) {
        ...
        if !self.heap.is_empty() {
            self.positions.insert(self.heap[0].clone(), 0);
            self.bubble_down(0);
        }
    }
    ...
    pub fn increase_priority(&mut self, item: &T) {
        let index = *self.positions.get(item).expect("item must exist");
    ...
    fn bubble_up(&mut self, mut index: usize) {
        while let Some(parent_idx) = self.parent(index) {
            if self.comparator.compare(&self.heap[index], &self.heap[parent_idx])
                == std::cmp::Ordering::Less
            {
                ...
                // Update positions
                self.positions.insert(self.heap[index].clone(), index);
                self.positions.insert(self.heap[parent_idx].clone(), parent_idx);
    ...
    fn bubble_down(&mut self, mut index: usize) {
        loop {
            let first_child_idx = self.first_child(index);
            
            if first_child_idx >= self.heap.len() {
                break;
            }

            // Find the child with minimum priority among all children
            let mut min_child_idx = first_child_idx;
            for i in 1..self.d {
                ...
                if self.comparator.compare(&self.heap[child_idx], &self.heap[min_child_idx])
                    == std::cmp::Ordering::Less
                {
            ...
            // If the minimum child has higher priority than current node, swap
            if self.comparator.compare(&self.heap[min_child_idx], &self.heap[index])
                == std::cmp::Ordering::Less
            {
                self.heap.swap(index, min_child_idx);
                
                // Update positions
                self.positions.insert(self.heap[index].clone(), index);
                self.positions.insert(self.heap[min_child_idx].clone(), min_child_idx);
...
mod tests {
    ...
    pub fn verify_heap_invariant(
        pq: &PriorityQueue<Item, impl PriorityCompare<Item>>,
    ) -> bool {
    ...
    fn insert_invariant_heap_property() {
        ...
        for item in items {
            ...
            assert!(
                front.priority <= 30,
                "front should have lowest priority value"
            );
        }
    ...
    fn insert_edge_becomes_front_if_highest_priority() {
        ...
        assert_eq!(
            pq.front().id,
            "high",
            "highest priority item should be at front"
        );
    ...
    fn front_size_unchanged() {
        ...
        assert_eq!(
            ...
            "size should be unchanged after front()"
        );
    }
    ...
    fn decrease_priority_position_item_moves_down() {
        ...
        assert_ne!(
            pq.front().id,
            "root",
            "item should move down after priority decrease"
        );
