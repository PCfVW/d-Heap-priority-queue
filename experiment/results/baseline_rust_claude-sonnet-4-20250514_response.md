Here's a complete implementation of a d-ary heap priority queue in Rust:
...
3. **O(1) lookup**: Uses a `HashMap` to track item positions for constant-time existence checks.

4. **Min-heap behavior**: Lower priority values have higher importance.

5. **All required operations**:
   - `insert()`: O(log_d n) - adds item and bubbles up
   - `pop()`: O(d log_d n) - removes root and bubbles down
   - `front()`: O(1) - returns reference to root
   - `increase_priority()`: O(log_d n) - decreases value and bubbles up
   - `decrease_priority()`: O(d log_d n) - increases value and bubbles down
   - `contains()`: O(1) - HashMap lookup
   - `len()` and `is_empty()`: O(1)

6. **Error handling**: Returns appropriate errors for invalid operations like duplicate insertions or invalid priority updates.

The implementation maintains the heap invariant that each parent has priority ≤ all its children, and uses the position map to enable efficient priority updates and lookups.