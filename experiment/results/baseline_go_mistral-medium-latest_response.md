...
Here's a complete implementation of a d-ary heap priority queue in Go that meets all your requirements:
...
2. **Item Identity and Priority**: Items have distinct identity (for equality) and priority (for ordering).
3. **O(1) Lookup**: Uses a map to track item positions for fast existence checks.
4. **Min-Heap**: Lower priority values have higher importance.
...
- The heap maintains the min-heap property where each parent's priority is ≤ its children's priorities.
- The `lookup` map provides O(1) existence checks and helps track item positions during heap operations.
- `heapifyUp` and `heapifyDown` maintain the heap property after insertions, deletions, and priority updates.
- The implementation handles edge cases like empty heap operations and invalid priority updates.

The example in `main()` demonstrates basic usage and the expected behavior of the priority queue.