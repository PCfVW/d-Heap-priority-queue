Here's a complete implementation of a d-ary heap priority queue in Zig:
...
1. **Configurable d-ary heap**: The arity `d` is set at construction time and must be ≥ 2.

2. **Item structure**: Items have separate `identity` and `priority` fields, with equality based only on identity.

3. **O(1) lookup**: Uses a HashMap to map identities to heap indices for constant-time existence checks.

4. **Min-heap semantics**: Lower priority values have higher importance and bubble to the top.

5. **All required operations**:
   - `insert()`: O(log_d n) - adds or updates items
   - `pop()`: O(d log_d n) - removes minimum priority item
   - `front()`: O(1) - peeks at minimum priority item
   - `increasePriority()`/`decreasePriority()`: O(log_d n) - updates priorities
   - `contains()`: O(1) - checks existence by identity
   - `len()`/`isEmpty()`: O(1) - size queries

6. **Duplicate handling**: Items with the same identity automatically update the existing entry's priority.

The implementation includes comprehensive tests covering basic operations, priority updates, duplicate handling, and different heap arities. The heap maintains the min-heap property where parent nodes have priority ≤ all their children, and the HashMap ensures O(1) lookups while keeping indices synchronized with heap operations.