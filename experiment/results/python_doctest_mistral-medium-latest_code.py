...
class DHeap:
    ...
    def __init__(self, d: int = 4):
        ...
        self.d = d
        self.heap: List[Item] = []
        self.position_map: Dict[int, int] = {}  # Maps item number to heap index
    ...
    def _children(self, index: int) -> List[int]:
        ...
        return list(range(start, min(start + self.d, len(self.heap))))

    def _bubble_up(self, index: int) -> None:
        """Move the item at index up to maintain heap property."""
        while index > 0:
            ...
            if self.heap[index].cost < self.heap[parent].cost:
                ...
                # Update position map
                self.position_map[self.heap[index].number] = index
                self.position_map[self.heap[parent].number] = parent
                index = parent
            else:
                break

    def _bubble_down(self, index: int) -> None:
        """Move the item at index down to maintain heap property."""
        while True:
            ...
            if self.heap[index].cost > self.heap[min_child].cost:
                ...
                # Update position map
                self.position_map[self.heap[index].number] = index
                self.position_map[self.heap[min_child].number] = min_child
                index = min_child
            else:
                break
    ...
    def insert(self, item: Item) -> None:
        ...
        if item.number in self.position_map:
            raise ValueError("Item with this identity already exists in the heap")
    ...
    def increase_priority(self, item: Item) -> None:
        ...
        if item.cost > self.heap[index].cost:
            raise ValueError("New cost must be lower than current cost for increase_priority")
    ...
    def decrease_priority(self, item: Item) -> None:
        ...
        index = self.position_map[item.number]
        if item.cost < self.heap[index].cost:
            raise ValueError("New cost must be higher than current cost for decrease_priority")
