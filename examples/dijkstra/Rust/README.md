...
Shortest path from A to F: A → C → E → F
Path cost: 9
Execution time: 34.4µs
```

## Performance Analysis

The results show the expected performance characteristics:
- **4-ary heap**: ~2x faster than binary heap
- **8-ary heap**: ~2.3x faster than binary heap
...
- Custom `Hash` and `Eq` implementations on `Vertex` enable O(1) priority updates based on vertex ID
